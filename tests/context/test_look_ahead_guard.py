...
def _candles(values: list[float]) -> list[RawCandle]:
    out = []
...
def test_as_of_matches_a_database_truncated_at_every_candle(tmp_path) -> None:
    ...
    for i in range(len(all_candles)):
        ...
        as_of_record = _evaluate_symbol(full_store, VENUE, SYMBOL, as_of)
        truncated_record = _evaluate_symbol(truncated_store, VENUE, SYMBOL, None)
